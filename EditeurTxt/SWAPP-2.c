...
void lireRepertoire(char* s, DIR* repertoire)
{   char nom[TAILLE_MAX];
    struct dirent* a=NULL;
     if (rep == NULL)
        exit(1);
     printf("** En cours de lecture '%s' **\n", s);
    while ((a= readdir(repertoire)) != NULL) /* Lecture du Repertoire. */
        printf("'%s'\n", a->d_name);
     ...
     charger2(f);//en cour de traitement
  }
